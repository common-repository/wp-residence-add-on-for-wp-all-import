    ...
    class WPAI_WP_Residence_Property_Fields {
        ...
        public function add_field( $field_type ) {
            switch( $field_type ) {
                ...
                case 'floor_plans':
                    $this->floor_plan_options();
                    $this->floor_plan_images();
                    break;
        ...
        public function floor_plan_options() {
            $floor_plan_options = array(
                $this->add_on->add_field( 'use_floor_plans', 'Use floor plans', 'radio', array(
                    '0' => 'No',
                    '1' => 'Yes'
                ) ),
                $this->add_on->add_field( 'plan_separator',   'Floor Plan Separator', 'text', null, 'Default separator is pipe symbol: |', true, '|' ),
                $this->add_on->add_field( 'plan_title',       'Plan Title',           'text', null, 'Separate multiple titles using the separator defined above, e.g.: Title A|Title B|Title C'),
                $this->add_on->add_field( 'plan_description', 'Plan Description',     'text', null, 'Separate multiple descriptions using the separator defined above, e.g.: Description A|Description B|Description C' ),
                $this->add_on->add_field( 'plan_size',        'Plan Size',            'text', null, 'Separate multiple sizes using the separator defined above, e.g.: 3000|6000|9000' ),
                $this->add_on->add_field( 'plan_rooms',       'Plan Rooms',           'text', null, 'Separate multiple rooms using the separator defined above, e.g.: 5|10|15' ),
                $this->add_on->add_field( 'plan_bath',        'Plan Bathrooms',       'text', null, 'Separate multiple bathrooms using the separator defined above, e.g.: 2|3|4' ),
                $this->add_on->add_field( 'plan_price',       'Plan Price',           'text', null, 'Separate multiple prices using the separator defined above, e.g.: 110000|150000|180000' )
            );

            $this->add_on->add_options( null, 'Floor Plans', $floor_plan_options );
        }

        public function floor_plan_images() {
            $this->add_on->import_images( 'wpai_res_add_on_import_floorplan_images', 'Floor Plan Images' );
        }
