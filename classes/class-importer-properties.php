...
if ( ! class_exists( 'WPAI_WP_Residence_Property_Importer' ) ) {
    class WPAI_WP_Residence_Property_Importer extends WPAI_WP_Residence_Add_On_Importer {
        ...
        public function import_advanced_options( $post_id, $data, $import_options, $article ) {
            ...
            // Import floor plans
            $this->import_floorplans( $post_id, $data, $import_options, $article );
        }        
        ...
        public function import_agent( $post_id, $data, $import_options, $article ) {
            ...
            if ( empty( $article['ID'] ) or $this->add_on->can_update_meta( $field, $import_options ) ) {
                ...
                if ( empty( $data[ $field ] ) ) {
                    ...
                    $post = wp_all_import_get_page_by_title( $data[ $field ], $post_type );
            ...
            if ( empty( $article['ID'] ) or $this->add_on->can_update_meta( $field, $import_options ) ) {
                ...
                if ( empty( $data[ $field ] ) ) {
                    ...
                    foreach ( $agents as $agent ) {
                        ...
                        $post = wp_all_import_get_page_by_title( $agent, $post_type );
        ...
        public function import_floorplans( $post_id, $data, $import_options, $article ) {            
            $fields = array(  
                'use_floor_plans',              
                'plan_title',
                'plan_description',
            ...
            $separator       = ( empty( $data['plan_separator'] ) ? '|' : $data['plan_separator'] );

            foreach ( $fields as $field ) {
                if ( empty( $article['ID'] ) || $this->add_on->can_update_meta( $field, $import_options ) ) {
                    if ( $field == 'use_floor_plans' ) {
                        $this->helper->update_meta( $post_id, $field, $data[ $field ] );
                    } else {
                        $import_data = explode( $separator, $data[ $field ] );
                        $import_data = array_filter( $import_data );
                        $this->helper->update_meta( $post_id, $field, $import_data );
                    }
                }
            }
    ...
    if ( ! function_exists( 'wpai_res_add_on_import_floorplan_images' ) ) {
        function wpai_res_add_on_import_floorplan_images( $post_id, $attachment_id, $image_filepath, $import_options ) {
            $wpresidence_addon      = new WPAI_WP_Residence_Add_On_Helper();
            $floorplan_image_fields = array( 'plan_image_attach', 'plan_image' );

            $save_floorplan_images_in_gallery = apply_filters( 'wpai_wp_residence_is_save_floorplan_images_to_gallery', false, $post_id, $attachment_id, $image_filepath, $import_options );

            if ( $save_floorplan_images_in_gallery === false ) {
                $attch_parent = get_post_field( 'post_parent', $attachment_id );
                if ( $attch_parent === $post_id ) {
                    set_post_field( 'post_parent', 0, $attachment_id );
                }
            }
            
            $existing_ids   = get_post_meta( $post_id, $floorplan_image_fields[0], true ); 
            $existing_urls  = get_post_meta( $post_id, $floorplan_image_fields[1], true );
            
            if ( ! empty( $existing_ids ) ) {
                $existing_ids   = maybe_unserialize( $existing_ids );
            } else {
                $existing_ids   = array();
            }

            if ( ! empty( $existing_urls ) ) {
                $existing_urls   = maybe_unserialize( $existing_urls );
            } else {
                $existing_urls   = array();
            }
            ...
            $existing_urls[] = wp_get_attachment_url( $attachment_id );
            $existing_urls   = array_filter( $existing_urls );
            
            $wpresidence_addon->update_meta( $post_id, $floorplan_image_fields[0], $existing_ids );
            $wpresidence_addon->update_meta( $post_id, $floorplan_image_fields[1], $existing_urls );
        }
    }
}
